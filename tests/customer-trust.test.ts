...
async function fixture() {
  const user = await prisma.user.create({
    data: { email: unique("trust@example.com"), name: "Trust Parent" },
  });
  ...
  const cart = await prisma.cart.create({
    ...
    include: {
      items: {
        include: {
          product: { include: { brand: true, images: true, inventory: true } },
        },
        orderBy: { id: "asc" },
      },
    },
...
describe("customer trust workflows", () => {
  ...
  beforeEach(async () => {
    await prisma.coupon.deleteMany({
      where: { normalizedCode: { startsWith: "TRUST" } },
    });
  });
  ...
  it("validates coupon date windows, free delivery and duplicate usage idempotently", async () => {
    ...
    expect(secureCartTotals(cart, result.finalCouponDiscount, 60).total).toBe(
      1000,
    );
  ...
  it("enforces delivered-order review eligibility and approved-only aggregates", async () => {
    ...
    await expect(
      assertReviewEligibility({ userId: user.id, orderItemId: orderItem.id }),
    ).resolves.toBeTruthy();
      ...
      data: {
        userId: (
          await prisma.user.create({
            data: { email: unique("hidden@example.com") },
          })
        ).id,
        productId: product.id,
        rating: 1,
  ...
  it("prevents over-refunds and keeps support internal notes private", async () => {
    ...
    const ticket = await prisma.supportTicket.create({
      data: {
        ...
        subject: "Help",
        messages: {
          create: { senderId: user.id, body: "Public question", public: true },
        },
        internalNotes: { create: { authorId: user.id, body: "Private note" } },
      },
      include: { messages: true, internalNotes: true },
    });
    const customerVisible = await prisma.supportMessage.findMany({

...
import {
  clearGuestCartTokenInAction,
  ensureGuestCartTokenInAction,
  readGuestCartToken,
} from "./security";
...
export async function updateCartItem(input: {
  ...
  const cart = input.userId
    ...
    : await (async () => {
        ...
        return (
          existing ??
          prisma.cart.create({ data: { guestToken }, include: cartInclude })
        );
      })();
  ...
  if (
    variant &&
    (!variant.active || variant.status !== ProductStatus.PUBLISHED)
  ) {
...
export function secureCartTotals(
  cart: CartView,
  discount = 0,
  deliveryFee = 80,
) {
  ...
  const delivery = Math.max(0, deliveryFee);
  return {
    subtotal,
    discount,
    delivery,
